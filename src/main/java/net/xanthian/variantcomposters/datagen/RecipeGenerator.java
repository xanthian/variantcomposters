...
public class RecipeGenerator extends FabricRecipeProvider {
    ...
    public void generate(RecipeExporter exporter) {
        ...
        //registerComposterRecipe(exporter, Ecologics.ECO_COMPOSTERS, "ecologics");
        //registerComposterRecipe(exporter, EldritchEnd.EE_COMPOSTERS, "eldritch_end");
        //registerComposterRecipe(exporter, Meadow.LDM_COMPOSTERS, "meadow");
        ...
        ShapelessRecipeJsonBuilder.create(RecipeCategory.DECORATIONS, Items.COMPOSTER,1)
                ...
                .offerTo(exporter,Identifier.of("variantcomposters", "composter"));
    ...
    public void registerComposterRecipe(RecipeExporter exporter, Map<Identifier, Block> composter, String modId) {
        registerComposterRecipe(exporter, composter, modId, "_slab");
    }
    public void registerComposterRecipe(RecipeExporter exporter, Map<Identifier, Block> composters, String modId, String slabSuffix) {
        for (Map.Entry<Identifier, Block> entry : composters.entrySet()) {
            ...
            if (firstUnderscoreIndex != -1 && lastUnderscoreIndex != -1 && lastUnderscoreIndex > firstUnderscoreIndex) {
                ...
                offerComposterRecipe(withConditions(exporter, ResourceConditions.and(ResourceConditions.allModsLoaded(modId), ResourceConditions.registryContains(RegistryKey.of(RegistryKeys.BLOCK, Identifier.of(slabPath))))), composter, Registries.ITEM.get(Identifier.of(slabPath)));
            } else {
